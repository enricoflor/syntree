...
** Create a new tree
Upon calling the command ~syntree~ a new frame is created that is
...
What the current style is right after ~syntree~ is called is determined
...
back in the position it was when ~syntree~ was called.  You can now
yank the tree.
